...
// Patch all per-game pages generated by generate-game-pages.js
// Structure: dist/game/{char}/{slug}/index.html
const gamePageGlob = path.join(dist, 'game');
if (fs.existsSync(gamePageGlob)) {
  let patched = 0;
  const charDirs = fs.readdirSync(gamePageGlob);
  for (const charDir of charDirs) {
    const charPath = path.join(gamePageGlob, charDir);
    if (!fs.statSync(charPath).isDirectory()) continue;
    const slugDirs = fs.readdirSync(charPath);
    for (const slug of slugDirs) {
      const p = path.join(charPath, slug, 'index.html');
      if (fs.existsSync(p)) {
  ...
  if (patched > 0) console.log(`✅  Patched __CACHE_VER__ in ${patched} game pages`);
}
